...
def plot_legend(legend):
    ...
    patches = [mpatches.Patch(color=(color[0] / 255, color[1] / 255, color[2] / 255), label=f"{name} ({train_id})")
...
def cityscapes_color_map():
    ...
    legend  = {label.train_id: (label.color, label.name) for label in classes if label.train_id not in [-1,255]}
...
def inference(net, image, new_size, plot = False, nn_thresh = 0.7, top_k = 4000, device='cuda'):
...
def estimatePose(kps_ref, kps_cur, cam):
    ...
    E, mask_match = cv2.findEssentialMat(kpn_cur, kpn_ref, focal=1, pp=(0., 0.), method=ransac_method, prob=0.999, threshold=0.0003)
...
def evaluate_visual_odometry(model, kitti_path, gt_path, video_name, device, new_size = None, plot = False, verbose= False):
    ...
    if model.nClasses == 19:
        ...
        legend = cv2.resize(legend, (int(resize*legend_ratio), resize), interpolation=cv2.INTER_NEAREST)
    else:
    ...
    while True:
        ...
        if plot:
            ...
            if model.nClasses == 19:
                segmentation_debug =cv2.cvtColor(color_map[out['seg'][0][0].cpu()].numpy(), cv2.COLOR_BGR2RGB)
            else:
                segmentation_debug = (color_map(out['seg'][0][0].cpu().numpy())* 255).astype(np.uint8)[...,:3]

            if 'depth' in out:
                segmentation_debug = cv2.resize(segmentation_debug, (size[1] // 2, size[0] // 2),
                                                interpolation=cv2.INTER_NEAREST)
                depth_debug = out['depth'][0][0].cpu().numpy()
                depth_debug = (color_map_depth(cv2.resize(depth_debug, (size[1]//2, size[0]//2)))[...,:3]* 255).astype(np.uint8)
                concatenated = np.hstack((segmentation_debug, depth_debug))
            else:
                concatenated = cv2.resize(segmentation_debug, (size[1] , size[0]),
                                                interpolation=cv2.INTER_NEAREST)
    ...
    if verbose:
        return {"translation": t_stats, "rotation": r_stats, "total": total_stats, "estimation_fails": estimation_fails}
    else:
        return total_stats
    # return error


def demo(model, video_path, cam_params, device, new_size=None, plot=False, verbose=False, track=False, out_path=None):
    # load images from video
    ...
    if track:
        video_out_key = cv2.VideoWriter(out_path + f'/output_key_track_{str(new_size[0])}_{str(new_size[1])}.mp4', fourcc, 30.0, (size[1], size[0]))
    else:
        video_out_key = cv2.VideoWriter(out_path + f'/output_key_{str(new_size[0])}_{str(new_size[1])}.mp4', fourcc, 30.0, (size[1], size[0]))
        video_out_seg = cv2.VideoWriter(out_path + f'/output_seg_{str(new_size[0])}_{str(new_size[1])}.mp4', fourcc, 30.0, (size[1], size[0]))
    while True:
        ret, frame = cap.read()
        # ret, frame = cap.read()
        # ret, frame = cap.read()
        ...
        if plot:
            ...
            if 'depth' in out:
                segmentation_debug = cv2.resize(segmentation_debug, (size[1] // 2, size[0] // 2),
                                                interpolation=cv2.INTER_NEAREST)
                depth_debug = out['depth'][0][0].cpu().numpy()
                depth_debug = (color_map_depth(cv2.resize(depth_debug, (size[1] // 2, size[0] // 2)))[...,
            ...
            if out_path is not None:
                ...
                if not track:
                    video_out_seg.write(cv2.resize(segmentation_debug, (size[1], size[0]),interpolation=cv2.INTER_NEAREST))
