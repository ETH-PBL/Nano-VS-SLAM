...
def evaluate_global_descriptor(
    model, eval_set, batch_size=4, device="cuda", num_workers=8
):
    ...
    for i, n in enumerate(n_values):
        ...
        print(
            "====> Recall@{}: {:.4f} AUC: {:.4f} MR: {:.4f}".format(
                n, recalls[n], auc[n], match_ratio[n]
            )
        )
