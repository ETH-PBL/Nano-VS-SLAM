...
from ..modules.decoders.segmentation import (
    SegmentationHead,
    SegmentationHeadATT,
    SegmentationFeatHeadLightATT,
    SegmentationFeatHeadLight,
)
...
def get_config(config, to_mcu=False, to_export=False, v3=False):
    ...
    if config not in config_dict:
        raise ValueError(
            "Config {} not supported, choose from ".format(config),
...
class KP2DTinyV2(torch.nn.Module):
    ...
    def __init__(
        ...
        depth=False,
        encoder_dim=None,
        global_descriptor_method="netvlad",
        **kwargs,
    ):
        super().__init__()
        self.device = device
        ...
        self.score_head = SimpleTaskHead(
            c4,
            c4,
            1,
            bn_momentum=self.bn_momentum,
        ...
        if self.use_attention:
            self.seg_head = SegmentationHeadATT(
                c4,
                c5,
                c4 + c3,
                nClasses,
            ...
            if self.depth:
                self.depth_head = SegmentationHeadATT(
                    c4,
                    c5,
                    c4 + c3,
                    1,
        ...
        self.vlad_head = VPRHead(
            ...
            remove_netvlad=remove_netvlad,
            leaky_relu=leaky_relu,
            method=global_descriptor_method,
        )
        if self.leaky_relu:
    ...
    def sample_seg(self, seg, coord_norm):
        if self.sample_segmentation:
            seg = torch.nn.functional.grid_sample(
                seg, coord_norm, align_corners=True, mode="nearest"
            )
    ...
    def normalize_coord(self, coord, H, W):
        ...
        coord_norm[:, 0] = (coord[:, 0] / (float(W - 1) / 2.0)) - 1.0
        coord_norm[:, 1] = (coord[:, 1] / (float(H - 1) / 2.0)) - 1.0
...
class KP2DTinyV3(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.score_loc_head = SimpleTaskHead(
            c4,
            c4,
            3,
            bn_momentum=self.bn_momentum,
        ...
        if use_attention:
            ...
            # else:
            self.seg_head = SegmentationFeatHeadLightATT(
                c4,
                c5,
                c4 + c3,
                nClasses,
            ...
            # else:
            self.seg_head = SegmentationFeatHeadLight(
                c4,
                c5,
                c4 + c3,
                nClasses,
