        ...
        if keypoint_mask is None:
            ref_desc = (
                torch.nn.functional.grid_sample(
                    source_des[cur_ind].unsqueeze(0),
            ...
            tar_desc = (
                torch.nn.functional.grid_sample(
                    target_des[cur_ind].unsqueeze(0),
            ...
            ref_desc = torch.nn.functional.grid_sample(
                source_des[cur_ind].unsqueeze(0),
                source_points[cur_ind].unsqueeze(0),
                align_corners=True,
            ).squeeze()[:, keypoint_mask_ind]
            tar_desc = torch.nn.functional.grid_sample(
                target_des[cur_ind].unsqueeze(0),
                tar_points[cur_ind].unsqueeze(0),
                align_corners=True,
            ).squeeze()[:, keypoint_mask_ind]
...
class KeypointNetwithIOLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Initialize Losses
        self.segmentation_criterion = torch.nn.CrossEntropyLoss(ignore_index=255).to(
            device
        )
        ...
        self.dice_loss = smp.losses.dice.DiceLoss(
            mode="multiclass", ignore_index=255
        ).to(device)
        self.vlad_criterion = HardTripletLoss(
            margin=0.5, hardest=True, squared=False
        ).to(device)
        ...
        if keypoint_net_type == "KeypointFormer":
            self.keypoint_net = KeypointFormer(
            ...
            self.keypoint_net = KP2DTinyV2(
                **conf, nClasses=self.n_classes, device=self.device, depth=load_depth
            )
        ...
        self.add_optimizer_params(
            "KeypointNet", self.keypoint_net.parameters(), keypoint_net_learning_rate
        )
        ...
        if self.with_io:
            ...
            self.add_optimizer_params(
                "InlierNet", self.io_net.parameters(), keypoint_net_learning_rate
            )
    ...
    def init_warper(self, size):
        self.warper = tgm.HomographyWarper(
            size[0] // self.d_f, size[1] // self.d_f, mode="nearest"
        )
    ...
    def forward(self, data, debug=False):
        ...
        if self.train_keypoints and self.keypoint_loss_weight > 0:
            ...
            source_uv_warped_norm = self._warp_homography_batch(
            ...
            d_uv_l2_min, d_uv_l2_min_index = _min_l2_norm(
                source_uv_warped, target_uv_pred, B
            )
            ...
            target_score_associated = (
                target_score.view(B, Hc * Wc)
                .gather(1, d_uv_l2_min_index)
                .view(B, Hc, Wc)
                .unsqueeze(1)
            )
            dist_norm_valid_mask = dist_norm_valid_mask.view(B, Hc, Wc).unsqueeze(
                1
            ...
            usp_loss = (
                ...
                + source_score[dist_norm_valid_mask]
            ...
            keypoint_loss += (
                self.score_loss_weight
                * torch.nn.functional.mse_loss(
                    target_score_resampled[border_mask.unsqueeze(1)],
                    source_score[border_mask.unsqueeze(1)],
                ).mean()
                * 2
            )
            if self.with_io:
                # Compute IO loss
                io_loss = self._compute_io_loss(
                    source_score,
                    source_feat,
                    target_feat,
                    target_score,
            ...
            loss_dict["metric_loss"] = 0.0
            loss_dict["usp_loss"] = 0.0
            loss_dict["io_loss"] = 0.0
        ...
        if self.train_segmentation and self.segmentation_loss_weight > 0:
            ...
            seg_loss += (
                self._compute_segmentation_loss(seg_aug, segmentation_gt_aug)
            ) * 0.5

            if self.use_mse_loss:
                seg_loss += (
                    self.MSE_Loss(
                        seg_aug.softmax(1), self.warper(seg, homography).softmax(1)
                    )
                ) * 1000
        ...
        if self.train_depth and self.depth_loss_weight > 0:
            ...
            depth_loss += (
                self.MSE_Loss(depth_aug, self.warper(depth, homography))
            ) * 0.5
        ...
        if debug or self.debug:
            if self.train_keypoints:
                ...
                vis_aug = (
                    input_img_aug[0].permute(1, 2, 0).detach().cpu().clone().squeeze()
                )
                vis_aug -= vis_aug.min()
                vis_aug /= vis_aug.max()
                ...
                _, top_k = source_score.view(B, -1).topk(
                    self.top_k2, dim=1
                )  # JT: Warped Source frame keypoints
                vis_ori = draw_keypoints(
                    vis_ori,
                ...
                vis_aug = draw_keypoints(
                    vis_aug,
                    source_uv_pred.view(B, 2, -1)[:, :, top_k[0].squeeze()],
                    (0, 0, 255),
                )
                ...
                vis_ori = np.clip(vis_ori, 0, 255) / 255.0
                vis_aug = np.clip(vis_aug, 0, 255) / 255.0
                heatmap = np.clip(heatmap * 255, 0, 255) / 255.0
                heatmap_aug = np.clip(heatmap_aug * 255, 0, 255) / 255.0
                ...
                concatenated_seg = np.vstack(
                    (np.hstack((heatmap, heatmap_aug)), np.hstack((vis_ori, vis_aug)))
                )
            ...
            if self.train_segmentation:
                seg_deb_warped = (
                    self.warper(data["seg"].clone().float().to(self.device), homography)
                    .squeeze(0)
                    .long()[0]
                    .permute(1, 2, 0)
                    .cpu()
                ...
                segmentation_debug = (
                    segmentation_gt[0].permute(1, 2, 0).cpu().numpy().astype(np.uint8)
                )
                segmentation_debug = (
                    cv2.resize(segmentation_debug, (W, H)) / self.n_classes
                )

                segmentation_debug_aug = (
                    segmentation_gt_aug[0]
                    .permute(1, 2, 0)
                    .cpu()
                    .numpy()
                    .astype(np.uint8)
                ...
                segmentation_debug_aug = (
                    cv2.resize(segmentation_debug_aug, (W, H)) / self.n_classes
                )
                ...
                segmentation_debug_pred = (
                    cv2.resize(segmentation_debug_pred, (W, H)) / self.n_classes
                )

                segmentation_debug_aug_pred = (
                    seg_aug[0]
                    .cpu()
                    .argmax(0)
                    .unsqueeze(0)
                ...
                segmentation_debug_aug_pred = (
                    cv2.resize(segmentation_debug_aug_pred, (W, H)) / self.n_classes
                )
                concatenated_seg = np.vstack(
                    (
                        np.hstack((segmentation_debug, segmentation_debug_aug)),
                        np.hstack(
                            (segmentation_debug_pred, segmentation_debug_aug_pred)
                        ),
                    )
                )
            ...
            if self.train_depth:
                depth_debug = (
                    depth[0].detach().cpu().numpy().squeeze().astype(np.float32)
                )
                depth_debug = cv2.resize(depth_debug, (W, H))
                depth_debug_aug = (
                    depth_aug[0].detach().cpu().numpy().squeeze().astype(np.float32)
                )
                ...
                concatenated_depth = np.vstack(
                    (
                        np.hstack((depth_gt, depth_gt_aug)),
                        np.hstack((depth_debug, depth_debug_aug)),
                    )
                )
    ...
    def _compute_io_loss(
        ...
        top_k_score1, top_k_indice1 = source_score.view(B, Hc * Wc).topk(
        ...
        top_k_score2, top_k_indice2 = target_score.view(B, Hc * Wc).topk(
            self.top_k2, dim=1, largest=False
        )
        ...
        source_uv_warped_norm_topk = source_uv_warped_norm[top_k_mask1].view(
            B, self.top_k2, 2
        )
        ...
        source_feat_topk = source_feat_topk.div(
            torch.norm(source_feat_topk, p=2, dim=1).unsqueeze(1) + epsilon
        )
        target_feat_topk = target_feat_topk.div(
            torch.norm(target_feat_topk, p=2, dim=1).unsqueeze(1) + epsilon
        )
        ...
        point_pair = torch.cat(
            [
                source_uv_norm_topk,
                target_uv_norm_topk_associated,
        ...
        return int(inlier_mask.sum() > 10) * torch.nn.functional.mse_loss(
            inlier_pred, inlier_gt
        )

    def _compute_segmentation_loss(self, pred, gt):
        return (
            self.segmentation_criterion(pred, gt.squeeze()) * 0.5
            + self.dice_loss(pred, gt.squeeze()) * 1.5
        )
    ...
    def _global_descriptor_loss(self, pred, pred_aug):
        ...
        for i in range(N):
            for j in range(N):
                if not i == j:
                    vlad_loss += self.vlad_criterion(
                        pred[i : i + 1], pred_aug[i : i + 1], pred[j : j + 1]
                    )
                    vlad_loss += self.vlad_criterion(
                        pred[i : i + 1], pred_aug[i : i + 1], pred_aug[j : j + 1]
                    )
                    vlad_loss += self.vlad_criterion(
                        pred_aug[i : i + 1], pred[i : i + 1], pred[j : j + 1]
                    )
                    vlad_loss += self.vlad_criterion(
                        pred_aug[i : i + 1], pred[i : i + 1], pred_aug[j : j + 1]
                    )
    ...
    def _compute_depth_loss(self, pred, gt):
        ...
        return (
            self.SILog_Loss(pred, gt, mask=mask)
            + self.HuberLoss(pred[mask], gt[mask]) * self.huber_loss_factor
        )  # + grad_loss + normal_loss
    ...
    def _warp_homography_batch(self, sources, homographies):
        ...
        for b in range(B):
            ...
            source = torch.addmm(
                homographies[b, :, 2], source, homographies[b, :, :2].t()
            )
...
def _normalize_uv_coordinates(uv_pred, H, W):
    ...
    uv_norm[:, 0] = (uv_norm[:, 0] / (float(W - 1) / 2.0)) - 1.0
    uv_norm[:, 1] = (uv_norm[:, 1] / (float(H - 1) / 2.0)) - 1.0
...
def _denormalize_uv_coordinates(uv_norm, H, W):
    ...
    uv_pred[:, :, :, 0] = (uv_pred[:, :, :, 0] + 1) * (float(W - 1) / 2.0)
    uv_pred[:, :, :, 1] = (uv_pred[:, :, :, 1] + 1) * (float(H - 1) / 2.0)
