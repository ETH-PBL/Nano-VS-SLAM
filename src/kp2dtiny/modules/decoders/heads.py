...
class SimpleTaskHead(Module):
    def __init__(
        self, c_in, c_hidden, c_out, bn_momentum=0.1, with_drop=False, leaky_relu=True
    ):
        super().__init__()
        self.convDa = AnnotatedConvBnReLUModel(
            c_in,
            c_hidden,
            kernel_size=3,
            stride=1,
...
class UpscaleHead(Module):
    def __init__(
        ...
        self.convA = AnnotatedConvBnReLUModel(
            c0,
            c1,
            kernel_size=3,
            stride=1,
        ...
        )  # TODO change to annotatedconvbnrelu
        self.confAa = AnnotatedConvBnReLUModel(
            c3,
            c4,
            kernel_size=3,
            stride=1,
