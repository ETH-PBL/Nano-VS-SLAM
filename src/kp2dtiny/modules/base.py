...
class AnnotatedConvBnReLUModel(nn.Module):
    def __init__(self, c0, c1, kernel_size=3, stride=1, padding=1, bias = False, bn_momentum=0.1, inplace=False, leaky_relu=True):
        super(AnnotatedConvBnReLUModel, self).__init__()
        self.conv = nn.Conv2d(c0, c1, kernel_size=kernel_size, stride=stride, padding=padding, bias = bias)
...
class ConvBnReLUModel(nn.Module):
    def __init__(self, c0, c1, kernel_size=3, stride=1, padding=1, bias=False, bn_momentum=0.1, inplace=True, leaky_relu=True):
...
class TransposedConvUpsampleModel(nn.Module):
    def __init__(self, c, kernel_size=3, stride=2, padding=2, bias=False, bn_momentum=0.1, inplace=True, leaky_relu=True):
        super(TransposedConvUpsampleModel, self).__init__()
        self.transposed_conv = nn.ConvTranspose2d(c, c//4, kernel_size=kernel_size, stride=stride, padding=1, output_padding=1, bias=bias)
...
class CustomPixelShuffle(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size, channels_div, self.upscale_factor, self.upscale_factor, in_height, in_width)
        ...
        return x.reshape(batch_size, channels_div, in_height * self.upscale_factor, in_width * self.upscale_factor)
