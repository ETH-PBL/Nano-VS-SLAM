...
import numpy as np
# from https://github.com/Nanne/pytorch-NetVlad/blob/master/netvlad.py
# based on https://github.com/lyakaap/NetVLAD-pytorch/blob/master/netvlad.py


def manual_normalize(x, dim=1, epsilon=1e-10):
    # Compute the L2 norm (magnitude) of the inputs along the specified dimension
    l2_norm = torch.sqrt(torch.sum(x**2, dim=dim, keepdim=True) + epsilon)
    # Manually divide by the norm to normalize
...
class NetVLAD(nn.Module):
    ...
    def forward(self, x):
        ...
        soft_assign = self.dequant(soft_assign)
...
# This one requires less memory but is slower
class NetVLADMemoryEfficient(nn.Module):
    """NetVLAD layer implementation"""
    ...
    def __init__(self, num_clusters=64, dim=128, normalize_input=True, vladv2=False):
        ...
        Args:
            ...
            vladv2 : bool
                If true, use vladv2 otherwise use vladv1
        """
        super(NetVLADMemoryEfficient, self).__init__()
        self.num_clusters = num_clusters
        self.dim = dim
        self.alpha = 0
        self.vladv2 = vladv2
