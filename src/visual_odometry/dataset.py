...
"""
# Adapted from Pyslam by luigi Freda
import sys
import numpy as np
...
class KittiDataset(Dataset):
    def __init__(self, path, name, associations=None, type=DatasetType.KITTI):
        ...
        self.set_is_color(self.check_color())
        print('Processing KITTI Sequence of lenght: ', len(self.timestamps))

    def check_color(self):
        if os.path.exists(self.path + '/sequences/' + self.name + '/image_0/') and os.path.exists(self.path + '/sequences/' + self.name + '/image_1/'):
            return False
        elif os.path.exists(self.path + '/sequences/' + self.name + '/image_2/') and os.path.exists(self.path + '/sequences/' + self.name + '/image_3/'):
            return True
        else:
            raise IOError('Could not find images in KITTI dataset: ', self.name, ' in path: ', self.path)
    ...
    def getImage(self, frame_id):
        ...
        if frame_id < self.max_frame_id:
            try:
                img = cv2.imread(
                    self.path + '/sequences/' + self.name + self.image_left_path + str(frame_id).zfill(6) + '.png')
    ...
    def getImage1(self, frame_id):
        ...
        if frame_id < self.max_frame_id:
            try:
                img = cv2.imread(
                    self.path + '/sequences/' + self.name + self.image_right_path + str(frame_id).zfill(6) + '.png')
                self._timestamp = self.timestamps[frame_id]
            except:
                print('could not retrieve image: ', frame_id, ' in path ', self.path)
            if frame_id + 1 < self.max_frame_id:
