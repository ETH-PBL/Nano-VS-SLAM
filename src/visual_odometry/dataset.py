...
class Dataset(object):
    def __init__(self, path, name, fps=None, associations=None, type=DatasetType.NONE):
        ...
        self._next_timestamp = (
            None  # next timestamp if available otherwise an estimate [s]
        )
...
class KittiDataset(Dataset):
    ...
    def check_color(self):
        if os.path.exists(
            self.path + "/sequences/" + self.name + "/image_0/"
        ) and os.path.exists(self.path + "/sequences/" + self.name + "/image_1/"):
            return False
        elif os.path.exists(
            self.path + "/sequences/" + self.name + "/image_2/"
        ) and os.path.exists(self.path + "/sequences/" + self.name + "/image_3/"):
            return True
        else:
            raise IOError(
                "Could not find images in KITTI dataset: ",
                self.name,
                " in path: ",
                self.path,
            )
