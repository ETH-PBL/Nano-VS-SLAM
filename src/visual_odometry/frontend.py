...
class KP2DtinyFrontend(object):
    ...
    def __init__(self, new_size,
               weights_path, nn_thresh=0.7,device="cuda", semantic_filter=False, classes_to_filter=[21], debug=True, method= 'kp2dtiny', config="A", top_k=4000, v3=False, nClasses=28):
        self.name = method
        self.device = device
        ...
        if self.name == 'kp2dtiny':
            self.net = tiny_factory(config, nClasses, to_export=False, to_mcu=False, v3=v3)
        ...
        if self.weights_path is not None:
            self.net.load_state_dict(torch.load(weights_path, map_location=torch.device("cpu"))['state_dict'])
    ...
    def get_info(self):
        return {'nn_thresh': self.nn_thresh,
    ...
    def run(self, img):
        ...
        with torch.no_grad():
            ...
            score, coord, feat, vlad, seg = out['score'], out['coord'], out['feat'], out['vlad'], out['seg']
