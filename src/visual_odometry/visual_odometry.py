...
class VisualOdometry(object):
    ...
    def __init__(self, cam, image_shape, new_size, conf={}):
        ...
        if self.use_lg:
            if self.method == "keypointnet":
                ...
                self.lg = (
                    LightGlue(
                        get_light_glue_config(self.conf["kp2dtiny_config"]),
                        self.conf["light_glue_path"],
                    )
                    .eval()
                    .to(self.device)
                )
    ...
    def init(self, image):
        ...
        if self.new_size is not None:
            image_tensor = kornia.geometry.transform.resize(
                image_tensor, size=self.new_size
            ).to(self.device)
    ...
    def match(self, image_tensor, kps_cur, feat_cur, seg_cur):
        if self.semantic_matching:
            ...
            kpts0_temp = (
                torch.tensor(self.prev_keypoints).unsqueeze(0).float().to(self.device)
            )
            ...
            if self.method == "keypointnet":
                kpts0_temp = kpts0_temp / torch.tensor(
                    [self.new_size[1], self.new_size[0]]
                ).unsqueeze(0).float().to(self.device)
                kpts1_temp = kpts1_temp / torch.tensor(
                    [self.new_size[1], self.new_size[0]]
                ).unsqueeze(0).float().to(self.device)
                image0 = {
                    "keypoints": kpts0_temp,
                    "descriptors": torch.tensor(self.prev_descriptors)
                    .unsqueeze(0)
                    .to(self.device),
                    "image_size": image_size,
                }
                image1 = {
                    "keypoints": kpts1_temp,
                    "descriptors": torch.tensor(feat_cur).unsqueeze(0).to(self.device),
                    "image_size": image_size,
                }
                with torch.no_grad():
                ...
                data = {
                    "keypoints0": kpts0_temp,
                    "descriptors0": torch.tensor(self.prev_descriptors)
                    .unsqueeze(0)
                    .to(self.device),
                    "view0": {"image_size": image_size},
                    "keypoints1": kpts1_temp,
                ...
                with torch.no_grad():
                    out = self.lg(data)
                m0, scores0 = (
                    out["matches0"].detach().cpu(),
                    out["matching_scores0"].detach().cpu(),
                )
                kps0, kps1, scores = get_matches_scores(
                    self.prev_keypoints, kps_cur, m0[0], scores0[0]
                )
                if (
                    scores.__len__() > self.conf["top_k_matches"]
                    and self.conf["top_k_matches"] > 0
                ):
            ...
            if (
                score.__len__() > self.conf["top_k_matches"]
                and self.conf["top_k_matches"] > 0
            ):
                top_k_idxs = np.argpartition(score, self.conf["top_k_matches"])[
                    : self.conf["top_k_matches"]
                ]
    ...
    def process_image(self, image, scale=1.0, conf=0.9):
        ...
        if self.method == "loftr":
            input = {
                "image0": self.prev_image.unsqueeze(0),
                "image1": image_tensor.unsqueeze(0),
            }
    ...
    def match_semantic(self, kps_cur, feat_cur, seg_cur):
        ...
        for class_id in range(28):
            ...
            try:
                idxs0, idxs1 = self.matcher.match(
                    self.prev_descriptors[class_idxs0], feat_cur[class_idxs1]
                )
    ...
    def estimatePose(self, kps_ref, kps_cur):
        ...
        self.stats.update(
            "num_outliers", self.mask_match.shape[0] - self.mask_match.sum()
        )
        return R, t
    ...
    def estimatePoseTorch(self, kps_ref, kps_cur):
        ...
        R, t, _ = kornia.geometry.epipolar.motion_from_essential_choose_solution(
            E, K, K, kp_cur_u, kp_ref_u
        )
