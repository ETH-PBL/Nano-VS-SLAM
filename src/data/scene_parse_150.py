...
def sample_homography(
        ...
        n_scales=100, n_angles=100, scaling_amplitude=0.2, perspective_amplitude=0.2,
        patch_ratio=0.7, max_angle=pi / 2):
...
def get_dataset(path, size, split="train", device="cpu", n_classes=7, ):
    ...
    transforms_post = v2.Compose([
        v2.ColorJitter(
            brightness=0.1, contrast=0.2, saturation=0.1, hue=0.05),
    ...
    def transforms(examples):
        transformed_images, transformed_masks, warped_images, warped_masks, homographies = [], [], [], [], []
        ...
        for image, seg_mask in zip(examples["image"], examples["annotation"]):
            ...
            image_t = apply_random_noise(transforms_post(image_t)).mul(2.).sub(1.)
            warped_image = apply_random_noise(transforms_post(warped_image)).mul(2.).sub(1.)
