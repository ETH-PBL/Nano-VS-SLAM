...
def get_nyuv2_transforms(im_h, im_w, d_f=4, n_classes=13, max_depth = 5000, min_depth = 0., val=False):
    ...
    if val:
        def transforms(examples):
            ...
            depth_t = transform_pre_seg(torch.tensor(np.array(depth)).unsqueeze(0)).float()
...
def get_nyuv2_depth_transforms(im_h, im_w, d_f=4, n_classes=13, max_depth = 5.0, min_depth = 0., val=False):
...
class NYUv2Dataset(Dataset):
    ...
    def __init__(self, path, n_classes=13, data_transform=None, split="train"):
        ...
        if self.path.exists():
            ...
            self.dataset = load_dataset("sayakpaul/nyu_depth_v2", split=split, cache_dir=Path(path)/"cache")
...
def apply_random_noise(img, noise_level=0.1, p=0.5):
    device = img.device
    return torch.clip(img + (torch.rand(1, device=device) > p) * noise_level * torch.randn_like(img), 0, 1.)
...
def get_dataset_nyuv2(path, split="train", device="cpu", size=(256,256), n_classes=40):
    ...
    norm = v2.Lambda(lambda img: img.div(255.))
    ...
    def transforms(examples):
        ...
        image = examples["image"]
        seg_mask = examples["annotation"]
        homography = torch.tensor(sample_homography(size)).view(1, 3, 3).float().to(device)

        image = image.convert('RGB')
        image_t = transform_pre(image).float().to(device)
        image_t = RGB_augmentations(image_t).unsqueeze(0)

        seg_mask_t = (transform_pre_seg(seg_mask).to(device, dtype=torch.uint8).unsqueeze(0) + 1).float()
        ...
        out["image"] = image_t.squeeze(0).mul(2.).sub(1.)
        out["image_aug"] = warped_image.squeeze(0).mul(2.).sub(1.)
    ...
    dataset = NYUv2Dataset_extracted(path, split=split, transform=transforms, n_classes=n_classes)
    return dataset
