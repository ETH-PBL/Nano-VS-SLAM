...
norm = transforms.Lambda(lambda img: img.mul(2.).sub(1.))
...
def get_whole_training_set(root,size, onlyDB=False, alternative_norm=True):
    ...
    return WholeDatasetFromStruct(root, structFile,
...
def get_training_query_set(root, size, margin=0.1, alternative_norm=True):
    ...
    return QueryDatasetFromStruct(root, structFile,
                             input_transform=input_transform(size, alternative_norm), margin=margin)
...
class QueryDatasetFromStruct(data.Dataset):
    def __init__(self, root_dir, structFile, nNegSample=1000, nNeg=10, margin=0.1, input_transform=None, nNegFactor = 10):
        super().__init__()
    ...
    def __getitem__(self, index):
        ...
        with h5py.File(self.cache, mode='r') as h5: 
            ...
            dNeg, negNN = knn.kneighbors(qFeat.reshape(1,-1), 
                    self.nNeg*self.nNegFactor) # to quote netvlad paper code: 10x is hacky but fine
            dNeg = dNeg.reshape(-1)
