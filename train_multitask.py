...
from utils.netvlad_utils import init_netvlad
...
def parse_args():
    ...
    parser.add_argument('--model_type', type=str, default="DD", help='Type of model [KeypointFormer, DD or DF]')
    parser.add_argument('--wandb_project', type=str, default="NanoVSSLAM", help='Wandb project name')
    ...
    parser.add_argument('--disable_strict_load', action='store_true', help='Disable strict load')
    parser.add_argument('--to_mcu', action='store_true', help='Train for MCU')
    parser.add_argument('--ignore_seg_head', action='store_true', help='Do not load segmentation head from checkpoint')
    parser.add_argument('--freeze_seg', action='store_true', help='Freeze seg head')
    parser.add_argument('--use_amp', action='store_true', help='Use automatic mixed precision (experimental)')
    parser.add_argument('--full_eval', default=3, type=int, help='Full evaluation every n epochs')
    ...
    parser.add_argument('--loss_weight_schedule', action='store_true', help='Use loss weight schedule')
    parser.add_argument('--only_segmentation', action='store_true', help='Only train segmentation')
    parser.add_argument('--only_keypoints', action='store_true', help='Only train keypoints')
...
def evaluate_local_keypoint_detection(model, dataloader, size):
    r, loc, c1, c3, c5, m, auc = evaluate_keypoint_net(dataloader, model, output_shape=size, top_k=300)
...
def main(args):
 ...
 ######################
    ...
    scheduler = {
        ...
        "plateau": torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.1, patience=5, verbose=True),
        "cosine": torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=2, eta_min=0),
        "none": None
    ...
    dataset, dataset_val = get_datasets(args, dataset_config, train_config, model, train_flags, size)
    ...
    if args.model_path is not None:
        ...
        model.keypoint_net.load_state_dict(state_dict, strict=not args.disable_strict_load)
    ...
    model.init_warper(size) #initialize Homography warper in the model (for homography augmentation)
    # NOTE
    ...
    if train_config['initial_evaluation']:
        start_results = initial_evaluation(args, model, dataset_config, dataloader_val, patches_dataloader, pittsburgh_dataset, train_config)
        print(start_results)
        if args.wandb:
    ...
    for epoch in range(args.start_epoch, train_config['n_epochs']):
        ...
        pbar = tqdm(enumerate(dataloader, 0), unit=' images', unit_scale=args.batch_size, total=len(dataloader), smoothing=0, disable=False)
        ...
        for i, sample in pbar:
            ...
            pbar.set_description(f'Train [ E {epoch} L {loss_2d:.4f}, S_L {loss_dict["seg_loss"]:.4f}, V_L {loss_dict["vlad_loss"]:.4f}]')
        ...
        results = evaluate_model(args, eval_model, dataloader_val, patches_dataloader, pittsburgh_dataset, train_flags, train_config, epoch, dataset_config)
...
def get_datasets(args, dataset_config, train_config, model, train_flags, size):
    if args.dataset_name == "scene_parse":
        dataset = get_dataset(dataset_config["scene_parse_data_path"], size, device=args.device, split="train", n_classes=train_config['n_classes'])
        dataset_val = get_dataset(dataset_config["scene_parse_data_path"], size, device=args.device, split="validation", n_classes=train_config['n_classes'])
    elif args.dataset_name == "cocostuff":
        dataset = COCOLoader(dataset_config["coco_data_path"], data_transform=get_coco_transforms(train_config['im_h'], train_config['im_w'], d_f=model.d_f, n_classes=train_config['n_classes'], load_depth=train_flags['depth']), split='train', depth=train_flags['depth'])
        dataset_val = COCOLoader(dataset_config["coco_data_path"], data_transform=get_coco_transforms(train_config['im_h'], train_config['im_w'], d_f=model.d_f, n_classes=train_config['n_classes'], val=True, load_depth=train_flags['depth']), split='val', depth=train_flags['depth'])
    elif args.dataset_name == "cityscapes":
        dataset = CityScapeLoader(dataset_config['cityscapes_data_path'], data_transform=get_cityscapes_transforms(train_config['im_h'], train_config['im_w'], d_f=model.d_f), split='train')
        dataset_val = CityScapeLoader(dataset_config['cityscapes_data_path'], data_transform=get_cityscapes_transforms(train_config['im_h'], train_config['im_w'], d_f=model.d_f, val=True), split='val')
    else:
        raise NotImplementedError("Dataset not implemented")
...
def initialize_wandb(args, config, model, dataset, dataset_val, train_flags):
    ...
    if train_flags['depth']:
        wandb_batch_val = torch.stack([dataset_val.__getitem__(i)['image'] for i in range(4)])
        wandb_batch_train = torch.stack([dataset.__getitem__(i)['image'] for i in range(4)])
        wandb_gt_val = torch.stack([dataset_val.__getitem__(i)['depth'] for i in range(4)])
        wandb_gt_train = torch.stack([dataset.__getitem__(i)['depth'] for i in range(4)])
...
def initial_evaluation(args, model, dataset_config, dataloader_val, patches_dataloader, pittsburgh_dataset, train_config):
    eval_model = model.keypoint_net
    eval_model.device = args.device
    eval_model.to(args.device)

    kitti_results = evaluate_visual_odometry(eval_model, dataset_config["kitti_path"], dataset_config["kitti_gt_path"], dataset_config["kitti_video_path"], args.device, (256, 1024), plot=args.debug)
    keypoint_results = evaluate_local_keypoint_detection(eval_model, patches_dataloader, (train_config['im_h'], train_config['im_w']))
    visloc_results = evaluate_global_descriptor(eval_model, pittsburgh_dataset, device=eval_model.device, num_workers=args.num_workers)
    segmentation_results = evaluate_segmentation(eval_model, dataloader_val, n_classes=train_config['n_classes'])

    return {"keypoints": keypoint_results, "visloc": visloc_results, "segmentation": segmentation_results, "vo": kitti_results}
...
def evaluate_model(args, eval_model, dataloader_val, patches_dataloader, pittsburgh_dataset, train_flags, train_config, epoch, dataset_config):
    results = {}
    ...
    if train_flags['depth']:
        depth_results = evaluate_depth_estimation(eval_model, dataloader_val, debug=args.debug)
        results["depth"] = depth_results

    if epoch % args.full_eval == 0:
        if train_flags['visloc']:
            vlad_recalls = evaluate_global_descriptor(eval_model, pittsburgh_dataset, device=eval_model.device, num_workers=args.num_workers)
...
def log_depth_examples(args, model, dataset, dataset_val):
    wandb_out = model.keypoint_net(torch.stack([dataset_val.__getitem__(i)['image'] for i in range(4)]).to(args.device))
    ...
    wandb_out = model.keypoint_net(torch.stack([dataset.__getitem__(i)['image'] for i in range(4)]).to(args.device))
    depth_image = wandb_out['depth'].detach().cpu()
    wandb_img = wandb.Image(depth_image, caption="depth")
