...
from utils.utils import load_checkpoint, set_seed, load_json, save_json
...
def parse_args():
    ...
    parser.add_argument(
        "--model_type", type=str, default="KeypointNet", help="Type of keypoint net"
    )
    parser.add_argument(
        "--wandb_project",
        type=str,
        default="MT-Evaluation-Seg",
    ...
    parser.add_argument(
        "--config", type=str, default="S", help="Model config [S, F, A, STM]"
    )
    ...
    parser.add_argument(
        "--visloc", action="store_true", help="Evaluate visual localization"
    )
    parser.add_argument(
        "--segmentation", action="store_true", help="Evaluate segmentation"
    )
    parser.add_argument(
        "--depth", action="store_true", help="Evaluate depth estimation"
    )
    ...
    parser.add_argument(
        "--backend", type=str, default="x86", help="Quantization backend"
    )
...
def evaluate_local_keypoint_detection(
    model, dataloader, size, debug=False, top_k_eval=300
):
    r, loc, c1, c3, c5, m, auc = evaluate_keypoint_net(
        dataloader, model, output_shape=size, top_k=top_k_eval, debug=debug
    )
    print(
        "Repeatability: {} Loc: {} C1: {} C3: {} C5: {} MScore: {}".format(
            r, loc, c1, c3, c5, m
        )
    )
    print(auc)
...
def main(args):
    ...
    if args.quantized:
        ...
        q_dl = torch.utils.data.DataLoader(
            q_ds,
    ...
    for size in resolutions:
        ...
        if dataset_name == "scene_parse":
            ...
            dataset_val = CityScapeLoader(
                ...
                data_transform=get_cityscapes_transforms(
                    size[0], size[1], d_f=d_f, val=True
                ),
                split="val",
            )
        elif dataset_name == "nyuv2":
            dataset_val = NYUv2Dataset_extracted(
                dataset_config["nyuv2_data_path"],
                data_transform=get_nyuv2_transforms(
                    size[0], size[1], d_f=d_f, val=True
                ),
                split="test",
            )
        ...
        pittsburgh_dataset = get_whole_val_set(
            dataset_config["pittsburgh_data_path"], size
        )
            ...
            for k in TOP_K:
                try:
                    keypoint_results = evaluate_local_keypoint_detection(
                        model, patches_dataloader, size, debug=args.debug, top_k_eval=k
                    )
        ...
        if args.segmentation and size[0]:
            try:
                segmentation_results = evaluate_segmentation(
                    model, dataloader_val, n_classes=args.n_classes, debug=args.debug
                )
        ...
        if args.depth and size[0] < 400:
            try:
                depth_results = evaluate_depth_estimation(
                    model, dataloader_val, debug=args.debug
                )
    ...
    if args.vo:
        for size in [(128, 256), (128, 512), (256, 1024)]:
            try:
                ...
                vo_results = evaluate_visual_odometry(
                    model,
