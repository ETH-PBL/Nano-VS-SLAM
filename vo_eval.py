...
from visual_odometry.dataset import KittiDataset
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Calculate relative error",
    )
    parser.add_argument(
        "--method", type=str, default="kp2dtiny", help="Feature extraction method"
    )
    parser.add_argument("--plot", action="store_true", help="Plot the 3D motion")
    parser.add_argument(
        "--draw_all", action="store_true", help="Draw all feature tracks"
    )
    parser.add_argument(
        "--semantic_filter", action="store_true", help="Use semantic filter"
    )
    parser.add_argument("--use_gt", action="store_true", help="Use ground truth")
    parser.add_argument(
        "--semantic_matching", action="store_true", help="Use semantic matching"
    )
    parser.add_argument(
        "--video_path",
        type=str,
        default="/home/thomas/PycharmProjects/pyslam_ubuntu/pyslam/videos/kitti06/",
        help="Path to the video",
    )
    parser.add_argument(
        "--gt_path", type=str, default="06.txt", help="Path to the ground truth file"
    )
    parser.add_argument(
        "--video_name", type=str, default="video_color.mp4", help="Name of the video"
    )
    parser.add_argument(
        "--device", type=str, default="cpu", help="Device to use for processing"
    )
    parser.add_argument(
        "--weights_path",
        type=str,
        default="./checkpoints/tiny_A.ckpt",
...
def main(args):
    ...
    if args.use_gt:
        ...
        error_type = (
            "relative error" if args.calculate_relative_error else "absolute error"
        )
    ...
    vo = VisualOdometry(
        ...
        {
            "device": args.device,
            "new_size": (args.im_h, args.im_w),
            "semantic_filter": args.semantic_filter,
            "semantic_matching": args.semantic_matching,
        },
    )
        ...
        if args.plot:
            if args.plot_traj:
                ...
                if args.use_gt:
                    ...
                    new_gt = (
                        np.array(x_gt - x_gt_0),
                        np.array(y_gt - y_gt_0),
                        np.array(z_gt - z_gt_0),
                    )
            ...
            start_time = end_time
            fps = 1.0 / elapsed_time.total_seconds()
            cv2.putText(
                img_debug,
                f"FPS: {fps}",
                (10, 30),
    ...
    save_json(
        artifacts, "./artifacts/vo_artifacts_" + args.method + "_" + timestamp + ".json"
    )
