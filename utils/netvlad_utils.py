...
def get_clusters(
    ...
    with h5py.File(initcache, mode="w") as h5:
        with torch.no_grad():
            ...
            for iteration, sample in enumerate(data_loader, 1):
                ...
                if iteration % 50 == 0 or len(data_loader) <= 10:
                    print(
                        "==> Batch ({}/{})".format(
                            iteration, ceil(nIm / cacheBatchSize)
                        ),
                        flush=True,
                    )
        ...
        niter = 100
        kmeans = faiss.Kmeans(
            model.encoder_dim, num_clusters, niter=niter, verbose=False
        )
        kmeans.train(dbFeat[...])
...
def init_netvlad(
    model, cluster_set, num_clusters=64, device="cuda", cacheDir="./cache"
):
    initcache = join(
        cacheDir, "centroids", "Scene_parse_" + str(num_clusters) + "_desc_cen.hdf5"
    )
    if exists(initcache):
        remove(initcache)
