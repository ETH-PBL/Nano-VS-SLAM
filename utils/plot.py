...
def create_legend(
    cmap, labels=load_json("src/data/cocostuff24.json")
):  # TODO: remove hardcoding
    # Create a figure and a subplot
    fig, ax = plt.subplots(
        figsize=(3, 10)
    )  # Adjust the figure size for a vertical legend
    num_labels = len(labels)

    # Generate a range of numbers which will be used for the labels
    data = torch.arange(num_labels).unsqueeze(
        1
    )  # Change to vertical by using unsqueeze(1)
    ...
    cbar.set_ticks(
        [i + 0.5 for i in range(num_labels)]
    )  # Adjust tick positions if labels are misaligned
    cbar.ax.set_yticklabels(
        [labels[str(i)] for i in range(num_labels)]
    )  # Map numerical labels to string labels in the JSON
