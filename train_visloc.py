...
def args_parse():
    ...
    parser.add_argument('--out_model_path', type=str, default="checkpoint.ckpt", help='Path to model checkpoint')
    ...
    parser.add_argument('--cacheDir', type=str, default='./cache', help='path to cache folder for storing precomputed descriptor pool')
    parser.add_argument('--n_classes', type=int, default=28, help='Number of classes in the training set')
    ...
    parser.add_argument('--model_type', type=str, default="KeypointNet", help='Model type')
    ...
    parser.add_argument('--to_mcu', action='store_true', help='Convert model to MCU format')
...
def get_clusters(model, args, cluster_set, initcache, nPerImage=100):
    ...
    data_loader = DataLoader(dataset=cluster_set, 
                num_workers=args.threads, batch_size=args.cacheBatchSize, shuffle=False, 
                pin_memory=False,
                sampler=sampler)
            ...
            for iteration, (input, indices) in enumerate(data_loader, 1):
                ...
                if iteration % 50 == 0 or len(data_loader) <= 10:
                    print("==> Batch ({}/{})".format(iteration, 
                        ceil(nIm/args.cacheBatchSize)), flush=True)
                del input, image_descriptors
        
        print('====> Clustering..')
        niter = 100
        kmeans = faiss.Kmeans(model.encoder_dim, model.num_clusters, niter=niter, verbose=False)
        kmeans.train(dbFeat[...])
    ...
    for subIter in range(subsetN):
        ...
        with h5py.File(train_set.cache, mode='w') as h5:
            ...
            h5feat = h5.create_dataset("features",
                                       ...
                                       dtype=np.float32)
            pbar = tqdm(enumerate(whole_training_data_loader, 1), total=len(whole_training_data_loader), unit_scale=args.cacheBatchSize)
            with torch.no_grad():
                for iteration, (input, indices) in pbar:
                    ...
                    pbar.set_description("====> Extracting Features {}/{}".format(iteration, len(whole_training_data_loader)))
                    del input, vlad_encoding, out

        sub_train_set = Subset(dataset=train_set, indices=subsetIdx[subIter])

        training_data_loader = DataLoader(dataset=sub_train_set, num_workers=8,
        ...
        pbar = tqdm(enumerate(training_data_loader, 1), total=len(training_data_loader), unit_scale=args.batch_size)
        for iteration, (query, positives, negatives,
                        negCounts, indices) in pbar:
            ...
            loss /= nNeg.float().to(args.device)  # normalise by actual number of negatives
            loss.backward()
            optimizer.step()
            ...
            pbar.set_description(("==> Epoch[{}][{}]({}/{}): Loss: {:.4f}".format(epoch, subIter, iteration,
                                                                  nBatches, batch_loss)))
...
if __name__ == '__main__':
    ...
    if args.model_type == 'KeypointFormer':
        ...
        model = KP2DTinyV2(**conf, nClasses=args.n_classes, mem_efficient=True, depth=args.depth)
    elif args.model_type == 'KP2DtinyV3':
        conf = get_config(args.config, v3=True, to_mcu=args.to_mcu)
        model = KP2DTinyV3(**conf, nClasses=args.n_classes, mem_efficient=True, depth=args.depth)
        alternative_norm = True
    else:
    ...
    whole_test_set = dataset.get_whole_val_set(dataset_config["pittsburgh_data_path"], size, alternative_norm=alternative_norm)
    whole_train_set = dataset.get_whole_training_set(dataset_config["pittsburgh_data_path"], size, alternative_norm=alternative_norm)
    whole_training_data_loader = DataLoader(dataset=whole_train_set, batch_size=args.cacheBatchSize, shuffle=False,
                                            pin_memory=True, num_workers=args.threads)

    train_set = dataset.get_training_query_set(dataset_config["pittsburgh_data_path"], size, margin=args.margin,alternative_norm=alternative_norm)


    history = None
    if args.model_path:
        state_dict, optimizer_state, history = load_checkpoint(args.model_path, optimizer_key="optimizer_visloc")
    ...
    initcache = join(args.cacheDir, 'centroids', whole_train_set.dataset + '_' + str(args.num_clusters) + '_desc_cen.hdf5')
    if exists(initcache):
        remove(initcache)
        
    cluster_set = dataset.get_whole_training_set(dataset_config["pittsburgh_data_path"], size, onlyDB=True)
    ...
    criterion = torch.nn.TripletMarginLoss(margin=args.margin ** 0.5,
                                     p=2, reduction='sum').to(args.device)
    evaluate_global_descriptor(model, whole_test_set, device=args.device, num_workers=args.threads)

    best_score = 0
    for i in range(args.nEpochs):
        train(i, args)

        results = evaluate_global_descriptor(model, whole_test_set, device=args.device, num_workers=args.threads)
