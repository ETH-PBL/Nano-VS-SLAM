...
from src.kp2dtiny.models.kp2d_former import (
    KeypointFormer,
...
def parse_args():
    ...
    parser.add_argument(
        "--to_export", default=True, help="Removes netvlad layer for export to MCU"
    )
...
if __name__ == "__main__":
    ...
    if args.model_type == "KP2Dtiny":
        model = KP2DTinyV2(
            **get_config(args.config, to_mcu=to_mcu, to_export=to_export),
            nClasses=args.n_classes,
        )
        name += "_" + args.config
    elif args.model_type == "KP2DtinyV3":
        model = KP2DTinyV3(
            **get_config(args.config, v3=True, to_mcu=to_mcu, to_export=to_export),
            nClasses=args.n_classes,
            depth=args.depth,
        )
        name += "_" + args.config
    ...
    if args.weight_path is not None:
        model.load_state_dict(
            torch.load(args.weight_path, map_location=torch.device("cpu"))["state_dict"]
        )
    ...
    torch.onnx.export(
        ...
        opset_version=16,
        input_names=["image"],
        output_names=["score", "coord", "desc", "vlad", "seg"],
